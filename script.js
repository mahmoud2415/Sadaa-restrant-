// ==========================================
// مطعم السعادة - script.js
// ==========================================
// 🔥 REFACTORED: Firebase is now the PRIMARY source for all menu data.
//    - Products, categories, and sections are fetched dynamically from Firestore.
//    - The fallback arrays below are used ONLY when Firebase is unreachable
//      (offline, slow network, or first load before cache warms up).
//    - Any product added via admin.html will appear here automatically
//      because startMenuRealtimeSync() subscribes to live Firestore updates.
//
// ✅ WHAT CHANGED:
//    1. Firebase is loaded first on DOMContentLoaded via hydrateAppFromFirebase().
//    2. A loading skeleton is shown while Firebase data is fetching.
//    3. An empty state is shown if Firebase returns zero products.
//    4. startMenuRealtimeSync() keeps the UI in sync with Firestore in real-time.
//    5. لا يوجد أي hardcode للمنتجات — Firebase هي المصدر الوحيد.
//
// ❌ WHAT WAS REMOVED (hardcoded data is no longer the primary source):
//    - The old inline product arrays that were directly rendered without Firebase.
//    - Any direct reference to menuData[] for rendering before Firebase loads.
//
// ==========================================
// 1. CONFIG
// ==========================================
...
// ==========================================
// 2. DATA — FALLBACK ONLY (emergency offline use)
// ==========================================
// ⚠️  DO NOT edit these arrays to manage menu content.
//     Use the admin panel (admin.html) instead.
//     These arrays exist only so the site doesn't break if Firebase is down.

// ==========================================
// 2. STATE — Firebase is the only source of truth
// ==========================================
// لا يوجد hardcode هنا — كل المنتجات بتيجي من Firebase فقط.
// عشان تضيف أو تعدل منتج، استخدم admin.html.

// Fallback arrays to prevent crash if Firebase fails
const fallbackMainSections = [];
const fallbackCategories = [];
const fallbackMenuData = [];
const fallbackFeaturedProducts = [];
// المصفوفات دي بتتملى من Firebase عن طريق hydrateAppFromFirebase()
let mainSections = [];
...
// buildMenuState: يبني حالة المنيو من بيانات Firebase فقط
// بعد حذف الـ hardcode، المصفوفات الابتدائية بتبقى فاضية
// والدالة دي بتملاها من الـ docs اللي جايين من Firestore
function buildMenuState(categoryDocs = [], productDocs = []) {
  ...
  // بناء الـ sectionMap من Firebase بس (مفيش fallback)
  ...
  // بناء الـ categoryMap من Firebase بس
  ...
  // بناء الـ productMap من Firebase بس
...
function buildFeaturedProducts(items) {
  // لو مفيش منتجات، ارجع مصفوفة فاضية
...
// restoreFallbackMenuData: بعد ما حذفنا الـ hardcode،
// الدالة دي بقت بتعمل reset للمصفوفات لحالة فاضية بس
// الموقع هيعرض loading state لحد ما Firebase ترد
function restoreFallbackMenuData() {
  mainSections = [...fallbackMainSections];
...
function syncLiveMenuData() {
  ...
  if (!loaded) {
    restoreCachedMenuData(); // آخر محاولة من الـ cache
  }
  refreshMenuUI();
}

/**
 * 🔥 REAL-TIME SYNC — This is the bridge between admin.html and the frontend.
 *
 * Subscribes to live Firestore updates on the "categories" and "products"
 * collections. Whenever an admin adds, edits, or deletes a product in
 * admin.html, Firestore fires an update, this function receives it, and
 * refreshMenuUI() re-renders the menu card grid automatically.
 *
 * No page refresh needed — products appear on the site within seconds.
 */
function startMenuRealtimeSync() {
  ...
  // Listen to the "categories" collection (sections + sub-categories)
  ...
  // Listen to the "products" collection (individual menu items)
  // Any product added from admin.html triggers this callback
...
function getQuickStartSectionDisplayName(section) {
  ...
  const quickStartLabels = {
    saada_sandwiches: 'ساندوتشات السعادة',
    saada_crepes: 'كريبات السعادة ',
    special_meals: 'التشيكن والبيف',
    sides_extras: 'مقبلات وإضافات'
  };
...
function getQuickStartReferenceImage(sectionId) {
  const referenceImageMap = {
    saada_sandwiches: 'ساندوتشات.jpeg' ,
    saada_crepes: 'كريبات.png',
    sides_extras: 'اضافات.jpeg',
    special_meals: 'تشيكن وبيف.jpeg'
  };
  ...
  return `
    <article class="section-card premium-section-card premium-section-card--compact ${theme} animate-slide-up" onclick="selectSection('${section.id}')">
      ...
      <div class="section-card-media premium-section-card__compact-media">
        <img src="${previewImage}" alt="${section.name}" loading="lazy" onerror="this.src='كريبات السعادة.jpeg'">
      </div>
    </article>
  `;
}
  ...
  return `
    <article class="section-card reference-section-card quick-start-card ${accentClass} ${index === activeIndex ? 'is-active' : ''}" onclick="selectSection('${section.id}')" onkeydown="if(event.key==='Enter' || event.key===' '){ event.preventDefault(); selectSection('${section.id}'); }" role="button" tabindex="0" aria-label="افتح قسم ${displayName}" data-quick-start-index="${index}">
      <div class="reference-section-card__media quick-start-card__media ${previewImage ? 'has-image' : 'is-icon-only'}">
        ${previewImage ? `<img src="${previewImage}" alt="${section.name}" loading="lazy" onerror="if(this.dataset.fallbackApplied==='1'){ this.closest('.quick-start-card__media').classList.add('is-icon-only'); this.remove(); } else { this.dataset.fallbackApplied='1'; this.src='${section.id === 'saada_crepes' ? 'كريبات.png' : 'hero-burger.png'}'; }">` : ''}
...
function initQuickStartCarousel() {
  ...
  track.addEventListener('pointermove', (event) => {
    ...
    if (Math.abs(deltaX) > 4) didPointerDrag = true;
...
function renderProducts(items) {
  ...
  if (!items || items.length === 0) {
    // Determine the right message depending on context
    const isSearching = Boolean(currentSearchTerm && currentSearchTerm.trim());
    const emptyMessage = isSearching
      ? `لا توجد نتائج مطابقة لـ "<strong>${currentSearchTerm}</strong>"`
      : 'لا توجد منتجات في هذا القسم حالياً — تابع الأخبار قريباً!';

    grid.innerHTML = `
      <div class="pcard-empty" role="status" aria-live="polite">
        <i class="fas fa-utensils" aria-hidden="true"></i>
        <p>${emptyMessage}</p>
        ${isSearching ? `<button onclick="clearMenuSearch()" class="btn-primary" style="margin-top:10px;padding:10px 20px;font-size:0.85rem;">
...
const trackOrderOverride = async function() {
  ...
  try {
    // 1. Search directly by Document ID if queryValue matches length 20 (Firestore document ID)
    ...
    // 2. Perform parallel optimized queries on Firestore
    const inputPhone = queryValue.replace(/\D/g, '');
    const uppercaseQuery = queryValue.toUpperCase();
    const queries = [];

    // Query by orderCode if it exists
    if (queryValue.length > 0) {
      queries.push(
        window.fsGetDocs(
          window.fsQuery(
            window.fsCollection(window.db, 'orders'),
            window.fsWhere('orderCode', '==', uppercaseQuery),
            window.fsLimit(5)
          )
        )
      );
    ...
    // Query by phone if a numeric phone is provided
    if (inputPhone) {
      queries.push(
        window.fsGetDocs(
          window.fsQuery(
            window.fsCollection(window.db, 'orders'),
            window.fsWhere('phone', '==', inputPhone),
            window.fsLimit(5)
          )
        )
      );
    ...
    snapResults.forEach(snap => {
      snap.docs.forEach(docSnap => {
        if (!matches.some(m => m.id === docSnap.id)) {
          matches.push({ id: docSnap.id, ...docSnap.data() });
        }
      });
    });

    // Sort matching orders by date desc
    matches.sort((a, b) => {
...
// ==========================================
// 🔥 FIREBASE HYDRATION — PRIMARY DATA LOADER
// ==========================================
// This is the main function that loads menu data from Firebase.
// It runs once on page load and then startMenuRealtimeSync() keeps
// the UI updated in real-time as admin adds/removes products.
...
async function hydrateAppFromFirebase() {
  ...
  // Show loading skeletons while Firebase fetches data
  showMenuLoadingSkeleton();

  try {
    // Step 1: Ensure the four fixed sections exist in Firestore
    // (runs silently on first visit to seed the database structure)
    await ensureRequiredSectionsExist();

    // Step 2: Fetch all categories + products from Firestore once
    // Firebase هي المصدر الوحيد للمنتجات — مفيش hardcode
    const loaded = await loadMenuFromFirebaseOnce();

    if (loaded) {
      // ✅ Firebase data loaded successfully — render it
      refreshMenuUI();
    } else {
      // ⚠️ Firebase returned empty — check localStorage cache
      // لو مفيش cache ومفيش Firebase data، الموقع هيبقى فاضي
      // والزبون هيشوف رسالة "لا توجد منتجات" بدل crash
      restoreCachedMenuData();
      refreshMenuUI();
    }

    // Step 3: Load business hours and update the hero badge
    const businessHours = await loadBusinessHours();
    renderHeroOpenStatus(businessHours);

    // Step 4: Subscribe to real-time Firestore updates
    // Any product added via admin.html will now appear automatically here
    startMenuRealtimeSync();

    // Hide the page loader AFTER Firebase data is loaded successfully
    hidePageLoader();

    return true;
  } catch (error) {
    // Firebase failed — عرض الـ cache لو موجود، غير كده الموقع يفضل فاضي
    restoreCachedMenuData();
    refreshMenuUI();
...
/**
 * Shows a skeleton loading state in the sections grid while
 * Firebase data is being fetched on first load.
 * Prevents layout shift and lets users know something is loading.
 */
function showMenuLoadingSkeleton() {
  const grid = document.getElementById('sections-grid');
  if (!grid) return;

  // Only show skeleton if no cached data was displayed already
  if (mainSections.length > 0) return;

  grid.innerHTML = `
    <div class="quick-start-carousel-track no-scrollbar" style="display:flex;gap:14px;padding:4px 2px;overflow:hidden;">
      ${Array(4).fill(0).map(() => `
        <div style="
          flex:0 0 240px;min-width:240px;height:220px;
          background:var(--bg-elevated,rgba(255,255,255,0.05));
          border:1px solid var(--border,rgba(255,255,255,0.08));
          border-radius:18px;
          display:flex;flex-direction:column;gap:10px;
          padding:16px;animation:skeletonPulse 1.4s ease-in-out infinite;
        ">
          <div style="width:60%;height:14px;background:rgba(255,255,255,0.08);border-radius:6px;"></div>
          <div style="width:90%;height:10px;background:rgba(255,255,255,0.05);border-radius:6px;"></div>
          <div style="flex:1;background:rgba(255,255,255,0.04);border-radius:12px;margin-top:auto;"></div>
        </div>
      `).join('')}
    </div>
  `;

  // Inject the skeleton pulse animation once
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ── STEP 1: عرض loading skeleton فوراً ──────────────────────────────────
  // مفيش hardcode دلوقتي — الموقع هيعرض skeleton لحد ما Firebase تجاوب
  // لو في cache من session سابق، هيتعرض لحظياً بدل الـ skeleton
  if (!restoreCachedMenuData()) {
    // مفيش cache — هنعرض skeleton في renderMainSections()
    restoreFallbackMenuData(); // بتخلي المصفوفات فاضية → هيظهر skeleton
  }

  // ── STEP 2: Render initial UI with whatever data we have ────────────────
  ...
  // Loader will be hidden inside hydrateAppFromFirebase() after data loads

  // ── STEP 3: Load fresh data from Firebase (primary source) ──────────────
  // If Firebase is already initialised, start loading immediately.
  // Otherwise, wait for the 'firebaseReady' event dispatched by index.html.
